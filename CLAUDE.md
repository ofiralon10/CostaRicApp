...
## Deployed Version
**App version: `0.58`** — **Hardened ItineraryPage against the recurring Madrid-wipe.** It no longer reads `shared/itinerary`; it always renders the built-in `src/data/itinerary.ts`. Root cause: the live chat backend still has a stale `functions/itinerary.js` (empty Madrid), so any in-app AI itinerary edit overwrites `shared/itinerary` with outdated data, masking the good file. Ignoring the shared doc makes the display immune. **Re-enable the `useSharedState('itinerary')` read in `ItineraryPage.tsx` once the functions are redeployed** with the synced `functions/itinerary.js` (the two AI tools + sync are committed, pending the `iam.serviceAccounts.ActAs` grant — see below).
_(prior: pending functions deploy)_ — Added two AI-chat tools in `functions/index.js`: **`set_day_title`** (rename a day → edits `location`/`locationHe`) and **`reorder_activities`** (reorder a day's events by activity-id list). Also **synced `functions/itinerary.js` from `src/data/itinerary.ts`** (it was stale — empty Madrid days + old IB flights — which would have re-corrupted the itinerary on the next AI edit). These are committed but **NOT live**: Cloud Functions need `firebase deploy --only functions`, which the CI service account can't do — it lacks `iam.serviceAccounts.ActAs` on the App Engine SA `costaricapp-2026@appspot.gserviceaccount.com`. **To enable CI functions deploys:** grant the deploy SA (the `firebase-adminsdk-…@costaricapp-2026.iam` account from the `FIREBASE_SERVICE_ACCOUNT` secret) the **Service Account User** role (`roles/iam.serviceAccountUser`) in GCP IAM, then re-run deploy.yml with the `deploy_functions` input. Or run `firebase deploy --only functions` locally once. **Until deployed, don't use the in-app AI chat to edit the itinerary** — the live backend still has the old itinerary copy and would revert the Madrid plan. `deploy.yml` now has optional `clear_itinerary` and `deploy_functions` workflow_dispatch inputs.
_(prior: `0.57`)_ — Added an **Accommodation** (🏨 `accommodation`) category to the Budget tracker's live spend buckets (`CATEGORIES` in `BudgetPage.tsx`); grid/breakdown/list all iterate dynamically so nothing else changed.
_(prior: `0.56`)_ — Enriched the Madrid days (16–19) with practical `notes`: food picks (San Miguel = grazing not dinner; Cava Baja tapas e.g. Txirimiri/Casa Lucio), getting-around logistics (Teleférico west ↔ Ikono south by metro), El Rastro pickpocket/early-start tip, Bernabéu metro L10 + arrive-early, airport transport (~30–40 min Metro/Cercanías or ~€30 taxi), and heat/AC guidance. Also added round-trip Teleférico advice (v0.55).
_(prior: `0.54`)_ — Swapped the Royal Palace (day 17, Sat Aug 8 morning) for the **Teleférico cable car + Casa de Campo** per family preference; rest of the Madrid plan unchanged.

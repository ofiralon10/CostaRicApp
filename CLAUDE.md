...
## Deployed Version
**App version: `0.52`** — **Itinerary activities are now checkable.** Each activity in `ItineraryPage` has a tap-to-toggle check (lucide `CheckCircle2`/`Circle`); done items get a strikethrough + green tint. State is **family-shared** via `useSharedState('activity-done')` (Firestore `shared/activity-done`, a `Record<activityId, true>` map — activity ids like `a2-0` are stable), so checking one off syncs to everyone. Each day header shows an `x/y` progress pill (green when all done). Layout: `.activity-item` is now a row (checkbox + `.activity-body` column); no Firestore rule change needed (`shared/{docId}` covers it).
